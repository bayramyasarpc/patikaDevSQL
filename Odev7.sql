...
SELECT DISTINCT(rating),title FROM film GROUP BY title,rating ORDER BY rating
...
SELECT MAX(country_id),COUNT(city) FROM city GROUP BY country_id ORDER BY COUNT(city) DESC LIMIT 1;
